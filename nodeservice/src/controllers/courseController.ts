...
export const getCourses = async (req: Request, res: Response): Promise<void> => {
  try {
    const { page = 1, pageSize = 10, subject, grade, title, status, userId } = req.query;
    const pageNum = parseInt(page as string, 10);
    const pageSizeNum = parseInt(pageSize as string, 10);

    const filters: any = {};
    if (subject && subject !== "all") filters.subject = { contains: subject as string };
    ...
    const { courses, totalCourses } = await getCoursesService(filters, skip, pageSizeNum);
...
export const updateLesson = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updatedCourse = await updateLessonService(courseId, lessonId, lessonData);
