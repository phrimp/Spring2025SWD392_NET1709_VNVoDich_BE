  ...
  if (tutor.availability) {
    return prisma.availability.update({
      ...
      data: {
        ...
        days: { deleteMany: {}, create: availabilityData },
      },
    });
  } else {
...
export const getCourseAvailabilityService = async (
  courseId: number,
  type: string
) => {
  const course = await prisma.course.findUnique({
    ...
    include: {
      tutor: {
        select: { availability: { select: { days: true, timeGap: true } } },
      },
      courseSubscriptions: {
        select: {
          teachingSessions: { select: { startTime: true, endTime: true } },
        },
      },
    },
  });
...
const generateAvailableTimeSlots = ({
  ...
  while (currentTime < slotEndTime) {
    ...
    const isSlotAvailable = !sessions.some(
      (booking) =>
